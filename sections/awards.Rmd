...
data <- rbind(
  ...
  tibble(
    Name = "Bourse d'aide à la participation à des rencontres scientifiques (UQAM)",
    URL = "",
...
data <- data %>% 
  ...
  mutate(Name = link(paste0("\\hspace{0.5cm} ", Name, "\\dotfill"), URL, color = "333333"),
         ...
         Amount = ifelse(Declined == TRUE,
                         paste0("(", Amount, ")"),
                         Amount))
data[18,1] <- gsub("333333", "6FA3CE", data[18,1]) #Red: FF0000, but we use same colour as rest
